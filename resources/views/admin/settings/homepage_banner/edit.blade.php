...
<div class="container-fluid">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form 
                action="{{ isset($banner) ? route('homepage_banner.update', $banner->id) : route('homepage_banner.store') }}" 
                method="POST" 
                ...
                <div class="form-group">
                    ...
                    <select name="sub_category_id" id="sub_category_id" class="form-control select2 @error('sub_category_id') is-invalid @enderror">
                        <option value="">-- Select Sub Category --</option>
                        @foreach($subCategories as $sub)
                            <option value="{{ $sub->id }}" {{ old('sub_category_id', $banner->sub_category_id ?? '') == $sub->id ? 'selected' : '' }}>
                                {{ $sub->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="child_sub_category_id" id="child_sub_category_id" class="form-control select2 @error('child_sub_category_id') is-invalid @enderror">
                        <option value="">-- Select Child Sub Category --</option>
                        @foreach($childSubCategories as $child)
                            <option value="{{ $child->id }}" {{ old('child_sub_category_id', $banner->child_sub_category_id ?? '') == $child->id ? 'selected' : '' }}>
                                {{ $child->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    @if(!empty($banner->banner))
                        <div class="mt-4">
                            <img src="{{ asset($banner->banner) }}" class="h-24" alt="Banner" height="80">
                        </div>
                    @endif
                </div>
