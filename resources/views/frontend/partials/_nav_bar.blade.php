@php
    $categories = \App\Models\Category::with('subcategories.child_sub_categories')->get();
@endphp
        ...
        <div class="absolute left-1/2 top-full z-50 w-[1100px] bg-white border shadow-2xl pl-4 pt-6 rounded-lg transform -translate-x-1/2 translate-y-[15px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
            <div class="flex gap-6">
                ...
                <div class="flex flex-col overflow-y-scroll overflow-x-hidden w-full max-h-[600px]" id="right_panel">
                    @foreach ($categories as $category)
                        <div class="right-content @if($loop->iteration !== 1) hidden @else first_category @endif" data-id="{{ $category->id }}">
                            @foreach ($category->subcategories as $subcategory)
                                <div class="mb-6">
                                    <h3 class=" font-semibold mb-2 underline text-[22px]">{{ $subcategory->name }}</h3>
                                    <ul class="grid grid-cols-6 gap-4">
                                        @foreach ($subcategory->child_sub_categories as $child)
                                            <li>
                                                <a href="#" class="text-sm hover:text-orange-500 flex flex-col gap-2 items-center text-center">
                                                    <img src="{{ asset($child->image) }}" class="rounded-full w-28 scale-90 hover:scale-110 transition-all object-fill" />
                                                    <span class="font-normal whitespace-normal">{{ $child->name }}</span>
                                                </a>
                                            </li>
                                        @endforeach
                                    </ul>
